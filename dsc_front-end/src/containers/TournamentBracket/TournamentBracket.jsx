  ...
  const updateMatchesWithResults = (results, currentMatches) => {
    ...
    if (finalRound && finalRound.score1 !== '-' && finalRound.score2 !== '-') {
      ...
      if (parseInt(finalRound.score1) === parseInt(finalRound.score2)) {
        tournamentWinner = parseInt(finalRound.penalty1) > parseInt(finalRound.penalty2)
          ? finalRound.team1
          : finalRound.team2;
      } else {
        tournamentWinner = parseInt(finalRound.score1) > parseInt(finalRound.score2)
          ? finalRound.team1
          : finalRound.team2;
      }
    ...
    try {
      ...
      if (Object.keys(convertedMatches).length > 0) {
        ...
        if (response && response.data && response.data.success) {
          ...
          // Cập nhật lại trạng thái matches và winner
          const updatedMatches = { ...matches };
          setMatches(updatedMatches);

          // Xác định lại winner từ trận chung kết (round1, match1)
          const finalMatch = updatedMatches.round1?.[0];
          if (finalMatch) {
            let winningTeam;
            const { score1, score2, penalty1, penalty2, team1, team2 } = finalMatch;

            if (parseInt(score1) === parseInt(score2)) {
              if (penalty1 && penalty2 && parseInt(penalty1) !== parseInt(penalty2)) {
                winningTeam = parseInt(penalty1) > parseInt(penalty2) ? team1 : team2;
              }
            } else {
              winningTeam = parseInt(score1) > parseInt(score2) ? team1 : team2;
            }
